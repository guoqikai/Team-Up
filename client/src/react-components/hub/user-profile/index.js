  ...
  render() {
    return (
      <div className="user-profile-pane">
        <div>
          <img src={this.state.picture ? this.state.picture : require("../../../static/user-default.png")} className="user-profile-pic" alt={this.state.username} />
          <div>
            <label for="user-profile-pic-input" className="user-profile-pic-button">change picture</label>
            <input type="file"  id="user-profile-pic-input" onChange={(e) => this.handlePicChange(e)}  accept="image/*" />
            </div>
        </div>
        <div className="user-profile-input-div">
          <span>email:</span>
          <input className="user-profile-text-input"
        ...
        <div className="user-profile-input-div">
        <span>about me:</span>
          <textarea className="user-profile-text-area"
            value={this.state.bio}
            onChange={(e) => this.handleTextFieldChange("bio", e)} />
        </div>
        <div><button className="button-common user-profile-send-button" onClick={() => this.sendUpdate()}>Save</button></div>
      </div>
    );
  }
}
...
function  wrappedUserProfile({}) {
  return <FetchWrapper fetchData={getCurrentUserInfo} private={true}><UserProfile/></FetchWrapper>;
}
